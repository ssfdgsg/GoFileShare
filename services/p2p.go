...
func (tc *TargetClient) establishP2PConnection() *quic.Conn {
	...
	// 第一步：建立统一的UDP Socket，配置socket选项以支持NAT穿透
	...
	// 拨号协程 - 作为客户端发起连接（实现simultaneous open）
	go func() {
		// 缩短延迟以实现同时连接，这对NAT穿透至关重要
...
// createReuseUDPConn 创建支持地址重用的UDP连接，这对NAT穿透至关重要
func (tc *TargetClient) createReuseUDPConn(laddr *net.UDPAddr) (*net.UDPConn, error) {
	// 使用ListenConfig来设置socket选项
	lc := net.ListenConfig{
		Control: tc.setSocketOptions,
	}
...
// continuousPunching 持续发送NAT打洞包直到QUIC连接建立或超时
func (tc *TargetClient) continuousPunching(ctx context.Context, conn *net.UDPConn, remoteAddr *net.UDPAddr, message []byte) {
