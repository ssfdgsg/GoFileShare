...
type TargetClient struct {
    ...
    LocalIP      string // 目标客户端内网IP（同一NAT时优先）
    LocalPort    string // 目标客户端内网端口
}
...
func InitInfo() (*P2PService, error) {
    ...
    for _, stunServer := range StunList {
        ...
        startNATKeepAlive(conn, serverAddr)
        return service, nil
    }

    // 所有STUN服务器均失败，关闭UDP连接
    _ = conn.Close()
...
func (p *P2PService) ConnectPeerTest(holePunchInfo *models.HolePunchInfo) error {
    ...
    addr := holePunchInfo.ExternalIP + ":" + holePunchInfo.ExternalPort
    if GlobalP2PClient != nil && holePunchInfo.ExternalIP == GlobalP2PClient.OutIP && holePunchInfo.LocalIP != "" && holePunchInfo.LocalPort != "" {
        addr = holePunchInfo.LocalIP + ":" + holePunchInfo.LocalPort
    }
...
func (tc *TargetClient) establishP2PConnection() quic.Connection {
    ...
    // 第一步：建立统一的UDP Socket（尽量复用STUN阶段打开的同一套接字，以保持外网映射一致）
    var udpConn *net.UDPConn
    var shouldClose bool
    if GlobalP2PClient != nil && GlobalP2PClient.UDPConn != nil {
        udpConn = GlobalP2PClient.UDPConn
        shouldClose = false
        log.Printf("复用已存在的UDP套接字: %s", udpConn.LocalAddr().String())
    } else {
    ...
    // 第二步：使用UDP Socket进行NAT穿透（同一公网IP时优先走内网地址，避免需要hairpin NAT）
    remoteHost := tc.ExternalIP
    remotePort := tc.ExternalPort
    if GlobalP2PClient != nil && GlobalP2PClient.OutIP == tc.ExternalIP && tc.LocalIP != "" && tc.LocalPort != "" {
        log.Printf("检测到同一公网IP，优先使用内网地址进行直连: %s:%s", tc.LocalIP, tc.LocalPort)
        remoteHost = tc.LocalIP
        remotePort = tc.LocalPort
    }
...
// startNATKeepAlive 使用STUN保活外网映射，避免NAT超时回收
func startNATKeepAlive(conn *net.UDPConn, serverAddr *net.UDPAddr) {
    ...
    go func() {
        ...
        defer ticker.Stop()
        // 使用Indication避免产生响应，降低对QUIC读循环的干扰
        msg := stun.MustBuild(stun.TransactionID, stun.BindingIndication)
        for range ticker.C {
            // 忽略错误：保活失败不致命，下次重试
